...
use ArmelWanes\Crudify\Annotation\Crudify;
...
class MakeManagedEntity extends AbstractMaker implements MakerInterface
{
    ...
    public function configureCommand(Command $command, InputConfiguration $inputConf)
    {
        $command
            ...
            ->addOption('crudify', null, InputOption::VALUE_NONE, 'Mark this class as Crudify')
    ...
    public function interact(InputInterface $input, ConsoleStyle $io, Command $command)
    {
        ...
        if (
              !$input->getOption('crudify')
        ) {
            $description = $command->getDefinition()->getOption('crudify')->getDescription();
            $question = new ConfirmationQuestion($description, false);
            $value = $io->askQuestion($question);

            $input->setOption('crudify', $value);
        }
        // crudify-setup
    ...
    public function configureDependencies(DependencyBuilder $dependencies, InputInterface $input = null)
    {
        ...
        if (null !== $input && $input->getOption('crudify')) {
            $dependencies->addClassDependency(
                Crudify::class,
                'crud'
            );
    ...
    {   $overwrite = $input->getOption('overwrite');
        ...
        $controllerExists = class_exists($controllerClassDetails->getFullName());
        if (!$classExists && !$controllerExists) {
            $entityPath = $entityClassGenerator->generateEntityClass(
                $entityClassDetails,
                $controllerClassDetails,
                $input->getOption('api-resource'),
                $input->getOption('crudify')
            );
        ...
        if ($classExists || $controllerExists) {
            ...
            $io->text([
                'Your entity or controller already exists! ',
            ]);
        } else {
            $io->text([
                '',
